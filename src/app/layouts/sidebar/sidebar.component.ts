...
import { admin, customer } from '../../config/app-menus';
...
export class SidebarComponent implements AfterViewChecked {
	...
	constructor(private router: Router, private route: ActivatedRoute, private authenticationService: AuthenticationService, private commonService: CommonService, private http: HttpClient, private eRef: ElementRef) {
		this.currentUserSubscription = this.authenticationService.currentUser.subscribe(user => {
			...
			if (Object.keys(this.currentUser).length > 0) {
				if((this.currentUser['custom:role']) == 'customer') {
					this.menus = customer
				} else if((this.currentUser['custom:role']) == 'admin') {
					this.menus = admin;
				}
			} 
			else {
				const getItem = JSON.parse(localStorage.getItem('currentStatus'));
				if(getItem && getItem.isLogout) {
		...
		this.commonService.subjectSubscription$.subscribe((data) => {
			if(data['custom:selectedPlan'] === 'Freemium-USD-Daily') {
				this.userCurrentPlan = `Trail`
			...
			} else if(data['custom:selectedPlan'] === 'Base-Plan-Monthly') {
				this.userCurrentPlan = `Base`
			} else if(data['custom:selectedPlan'] === 'Pro-Plan-Monthly') {
				this.userCurrentPlan = `Pro`
			} else { // This is enterprise account
