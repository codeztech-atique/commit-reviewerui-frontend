...
import { AppRoutingModule } from './routes/app-routing.module'; // Import your AppRoutingModule
...
export class AppComponent implements OnInit, OnDestroy {
	...
	routeObserverSubscription: Subscription;
	appSettings;
	...
	constructor(private titleService: Title, private introService: IntroService, private router: Router, private renderer: Renderer2, private authenticationService: AuthenticationService, private appRoutingModule: AppRoutingModule) {
		...
		this.routeObserverSubscription = this.router.events.subscribe((e) => {
			if (e instanceof NavigationStart) {
				// this.wbsocketService.receiveNotificationMessage(); // change later
				if (e.url === '/otp') {
					this.showChat = false;
				} else {
	...
	ngOnDestroy() {
		this.routeObserverSubscription.unsubscribe();
	}
